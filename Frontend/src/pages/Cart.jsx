...
const fetchProducts = async () => {
  try {
    const res = await axios.get(`https://fanshoebackend.onrender.com/apiv1/productsfromcart`);
...
const handlePayment = async (amount ,  products , user  , updatorFunc)=>{
  try {
    const res = await axios.post(`https://fanshoebackend.onrender.com/apiv1/order/`,{ amount:amount*100 , notes:products , receipt: `${Math.floor(Math.random()*999999)+999999}`})
    if (!res.data) throw new Error("No data found");
    //  console.log(res.data);
     var option = {
      ...
      handler:async function(response) {
        ...
        const body = {...response};
        const isValidPayment = await axios.post('https://fanshoebackend.onrender.com/apiv1/validate',{...body , amount , products , user});
